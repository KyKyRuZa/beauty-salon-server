...
const register = async (req, res) => {
  logger.info('Получен запрос на регистрацию администратора', {
    ip: req.ip,
  ...
  try {
    ...
    if (!phoneRegex.test(adminData.phone)) {
      return res.status(400).json({
        success: false,
        message:
          'Номер телефона должен содержать не менее 10 символов и может включать цифры, пробелы, дефисы и скобки',
      });
    }
    ...
    logger.info('Администратор успешно зарегистрирован', {
...
const login = async (req, res) => {
  logger.info('Получен запрос на вход администратора', {
    ip: req.ip,
    userAgent: req.get('User-Agent'),
  });
