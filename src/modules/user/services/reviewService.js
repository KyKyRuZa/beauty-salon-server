  ...
  try {
    ...
    if (booking_id) {
      ...
      if (booking.user_id !== user_id) {
        logger.warn('Пользователь пытается оставить отзыв к чужому бронированию', {
          user_id,
          booking_id,
        });
...
const updateMasterRating = async (masterId, transaction = null) => {
  try {
    ...
    await Master.update({ rating: newRating }, { where: { id: masterId }, ...options });
...
const updateSalonRating = async (salonId, transaction = null) => {
  try {
    ...
    await Salon.update({ rating: newRating }, { where: { id: salonId }, ...options });
